...
mod tests {
    ...
    fn setup(env: &Env) -> (ContractClient, Address, Address, Address) {
        ...
        // register a mock token so prize deposit doesn't panic on token transfer
        ...
        let seed_hash_bytes = env.crypto().sha256(
            &Bytes::from_array(env, &42u64.to_be_bytes())
        );
        ...
        (client, creator, admin, token_addr)
    }

    fn make_seed_hash(env: &Env, seed: u64) -> BytesN<32> {
        env.crypto()
    ...
    #[test]
    fn commit_seed_happy_path() {
        let env = Env::default();
        let (client, creator, _, _) = setup(&env);

        let hash = make_seed_hash(&env, 42);
        client.commit_seed(&creator, &hash); // must not panic
    }

    #[test]
    #[should_panic]
    fn commit_seed_rejects_duplicate() {
        let env = Env::default();
        let (client, creator, _, _) = setup(&env);

        let hash = make_seed_hash(&env, 42);
        client.commit_seed(&creator, &hash);
        client.commit_seed(&creator, &hash); // second call must fail
    }

    #[test]
    #[should_panic]
    fn commit_seed_rejects_non_creator() {
        let env = Env::default();
        let (client, _, _, _) = setup(&env);

        let stranger = Address::generate(&env);
        let hash = make_seed_hash(&env, 42);
        client.commit_seed(&stranger, &hash);
    }
    ...
    #[test]
    fn reveal_seed_happy_path() {
        let env = Env::default();
        let (client, creator, _, _) = setup(&env);

        let seed: u64 = 99;
        let hash = make_seed_hash(&env, seed);
        client.commit_seed(&creator, &hash);

        // advance one ledger so sequence > commit ledger
        env.ledger().set(LedgerInfo {
            sequence_number: 101,
            ..default_ledger_info()
        });

        client.reveal_seed(&creator, &seed); // must not panic
    }

    #[test]
    #[should_panic]
    fn reveal_seed_rejects_early_reveal_same_ledger() {
        let env = Env::default();
        let (client, creator, _, _) = setup(&env);
        ...
        let hash = make_seed_hash(&env, seed);
        client.commit_seed(&creator, &hash);
        // do NOT advance ledger — same sequence as commit
        client.reveal_seed(&creator, &seed);
    }

    #[test]
    #[should_panic]
    fn reveal_seed_rejects_wrong_seed() {
        let env = Env::default();
        let (client, creator, _, _) = setup(&env);
        ...
        client.reveal_seed(&creator, &999u64); // wrong seed, hash won't match
    }

    #[test]
    #[should_panic]
    fn reveal_seed_without_prior_commit() {
        let env = Env::default();
        let (client, creator, _, _) = setup(&env);
        client.reveal_seed(&creator, &42u64);
    }

    // -------------------------------------------------------------------------
    // finalize_commit_reveal tests
    // -------------------------------------------------------------------------

    fn do_commit_reveal(env: &Env, client: &ContractClient, creator: &Address, seed: u64) {
        let hash = make_seed_hash(env, seed);
        client.commit_seed(creator, &hash);
        env.ledger().set(LedgerInfo {
            sequence_number: 101,
    ...
    #[test]
    #[should_panic]
    fn finalize_commit_reveal_without_reveal() {
        let env = Env::default();
        let (client, creator, _, _) = setup(&env);
    ...
    #[test]
    #[should_panic]
    fn finalize_commit_reveal_wrong_source_panics() {
        // raffle with Internal source — finalize_commit_reveal must reject it
        let env = Env::default();
        env.ledger().set(default_ledger_info());
        env.mock_all_auths();
